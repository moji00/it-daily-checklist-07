...
const Dashboard: React.FC = () => {
  ...
  const downloadPDF = async () => {
    ...
    const filename = `IT_Checklist_${today}_${user?.full_name?.replace(/\s+/g, '_') || user?.username}.pdf`;
  ...
  const downloadExcel = () => {
    ...
    const filename = `IT_Checklist_${today}_${user?.full_name?.replace(/\s+/g, '_') || user?.username}.xlsx`;
